...
real(dl) mass_correctors(Params%Nu_mass_eigenstates)
...
!also need v2, define phiosc_2 at the top
...
!!! Added by Yang
open(1, file = 'littleh.dat', status = 'replace')
...
do i=2,ntable,1
   ...
   !littlehfunc=(omegah2_regm/(a_arr(i)**3.0d0)+Params%omegah2_rad/(a_arr(i)**4.0d0))+&
	!&sum(lhsqcont_massive*mass_correctors,Params%Nu_mass_eigenstates)/(a_arr(i)**4.0d0)

!Added by Yang
!!!print*, lhsqcont_massive*mass_correctors,Nu_mass_eigenstates


!Contributions from cosmological constant and scalar field
!littlehfunc=littlehfunc+omegah2_lambda+((v_vec(2,i)/a)**2.0d0)
!littlehfunc=littlehfunc+(maxion_twiddle*v_vec(1,i))**2.0d0
!ittlehfunc=littlehfunc*(a_arr(i)**2.0d0)+omk*hsq

   !print*,'fortran', maxion_twiddle !(omegah2_regm/(a_arr(i)**3.0d0)+Params%omegah2_rad/(a_arr(i)**4.0d0))!+&
   !     &sum(lhsqcont_massive*mass_correctors,Params%Nu_mass_eigenstates)/(a_arr(i)**4.0d0)+omk*hsq
   !, ((v_vec(2,i)/a_arr(i))**2.0d0)+&
   !&(maxion_twiddle*v_vec(1,i))**2.0d0


!!!!!!Added by Yang
  ! print*,'littlehfunc=', littlehfunc(i)
   write(1,*) a_arr(i),  littlehfunc(i),  v_vec(1,i), v_vec(2,i)
...
do i=1,Params%Nu_mass_eigenstates,1
call Nu_rho(a_arr(i)*Nu_masses(i),rhonu)
mass_correctors(i)=rhonu
enddo

!!!Added by Yang
open(2, file = 'aosc_param.dat', status = 'replace')
!!!!!

!!!Added by Yang
!write(*,*) i,ntable
!write(*,*) lhsqcont_massive, mass_correctors
!write(*,*) Params%Nu_mass_eigenstates
write(2,*) v_vec(1:2,i), omegah2_regm, Params%omegah2_rad, omegah2_lambda, &
     maxion_twiddle, omk*hsq, Params%a_osc, lhsqcont_massive, Params%Nu_mass_eigenstates, mass_correctors, phiosc, phidosc
!print*, 'v_vec(2,i), v_vec(1,i)', v_vec(2,i), v_vec(1,i)
!print*, 'phiosc, phidosc', phiosc, phidosc
print*,'omegah2_regm=', omegah2_regm
print*,'Params%omegah2_rad=', Params%omegah2_rad
print*,'omegah2_lambda=', omegah2_lambda
print*,'maxion_twiddle=', maxion_twiddle
!print*,'omk*hsq=', omk*hsq
!print*,'a_arr(i)=', a_arr(i)
!print*,'lhsqcont_massive=', lhsqcont_massive
!print*,'Params%Nu_mass_eigenstates=', Params%Nu_mass_eigenstates
!print*,'mass_correctors=', mass_correctors


!omegah2_regm, Params%omegah2_rad, omegah2_lambda, &
!maxion_twiddle, omk*hsq, a_arr(i),

!, lhsqcont_massive, Params%Nu_mass_eigenstates, mass_correctors
!sum(lhsqcont_massive*mass_correctors,Nu_mass_eigenstates) mass_correctors(i),
...
!littlehfunc=littlehfunc+omegah2_lambda+((v(2)/a)**2.0d0)
!littlehfunc=littlehfunc+(maxion_twiddle*v(1))**2.0d0
!littlehfunc=littlehfunc*(a**2.0d0)+omk*hs
