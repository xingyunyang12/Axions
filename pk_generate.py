...
fa_list = np.linspace(0.01,0.1,10)

for k in range(len(fa_list)-1):
    l1 = []
    ...
    for i in range(8):
        ...
        input_file_path = 'input_file_%s.txt' %(j)
        try:
        ...
        axionCAMB_exe_path = '../axionHMcode/axionCAMB'
        ...
        l1.append(data_ax)
        # datafile_path = "nonlin_PS_with_axion_%s.txt" %(j) #change path if you want
        # np.savetxt(datafile_path , data_ax)
        # print(input_file_path, type(input_file_path))
        #
        # old_fa = str(old_fa)
        # fa = str(fa)
        ...
        new_text = text.replace('ax_fraction = %s' %(old_fa), 'ax_fraction = %s' %(fa))
...
#Yang's edit
#plot non-linear power spectrum
#plt.plot(power_spec_dic_ax['k'], PS_matter_nonlin[0], label=r'non-linear power spectrum in axion DM cosmology')
...
#plt.legend()
#plt.show()
# plt.savefig('matter_power_axionHM.pdf')
# plt.close()
